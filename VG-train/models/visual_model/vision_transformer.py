    ...
    def forward(self, x: torch.Tensor, 
        ...
        if self.fused_attn:
            ...
            if key_padding_mask is not None:
                attn_mask1 = key_padding_mask.unsqueeze(1).expand(-1, N, -1)
                attn = attn.masked_fill(attn_mask1.unsqueeze(1), float('-inf'))
            attn = attn.softmax(dim=-1)
            attn = attn.masked_fill(attn_mask1.transpose(1, 2).unsqueeze(1), 0)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.num_suffix_tokens = 1 if distillation_token else 0
        ...
        self.dstl_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim)) if distillation_token else None
        self.reg_token = nn.Parameter(torch.zeros(1, reg_tokens, embed_dim)) if reg_tokens else None
        embed_len = num_patches if no_embed_class else num_patches + self.num_prefix_tokens + self.num_suffix_tokens
